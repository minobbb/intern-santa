...
const game = () => {
  ...
  function Snowman() {
    ...
    hat.rotation.x = -80;
    hat.position.z = 33 * zoom;
    hat.castShadow = true;
    hat.receiveShadow = false;
    snowman.add(hat);
    ...
    leftArm.position.x = -20;
    leftArm.position.z = 15 * zoom;
    ...
    rightArm.rotation.y = -10;
    rightArm.position.x = 20;
    rightArm.position.z = 15 * zoom;
  ...
  function Grass() {
    ...
    const middle = createSection(0xdddddd);
    middle.receiveShadow = true;
    grass.add(middle);

    const left = createSection(0xcccccc);
    left.position.x = -boardWidth * zoom;
    grass.add(left);

    const right = createSection(0xcccccc);
    right.position.x = boardWidth * zoom;
    grass.add(right);
    ...
    if (stepStartTimestamp) {
      ...
      switch (moves[0]) {
        case 'forward': {
          ...
          snowman.position.y = positionY; // initial snowman position is 0

          snowman.position.z = jumpDeltaDistance;
          break;
        }
        case 'backward': {
          let positionY =
        ...
        case 'left': {
          ...
          snowman.position.x = positionX; // initial snowman position is 0
          snowman.position.z = jumpDeltaDistance;
          break;
        }
        case 'right': {
      ...
      lanes[currentLane].vechicles.forEach((vechicle) => {
        ...
        if (
          ...
          document.getElementById('result-score').innerText =
            parseInt(currentLane);
          document.getElementById('result-coin').innerText = parseInt(
            currentLane / 2,
          );
